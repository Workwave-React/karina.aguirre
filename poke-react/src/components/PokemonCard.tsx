...
import { Card, CardContent, Typography, CircularProgress, Box, Checkbox } from "@mui/material";
...
function PokemonCard({ name }: Props) {
  ...
  const isComparing = isInCompare(name);
  ...
  const handleCompareToggle = (e: React.ChangeEvent<HTMLInputElement>) => {
    e.stopPropagation();
    console.log("Toggle clicked!", name, "Current list:", compareList);
    if (isComparing) {
      ...
      console.log("Cannot add more, list is full");
    }
  };
  ...
  return (
    <Box sx={{ position: "relative", width: "100%" }}>
      <Box
        sx={{ position: "absolute", top: 20, right: 5, zIndex: 10, }}
      >
        <Checkbox
          checked={isComparing}
